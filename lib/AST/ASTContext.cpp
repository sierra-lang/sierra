...
/// of the specified element type and size. VectorType must be a built-in type.
QualType ASTContext::getSierraVectorType(QualType T,
                                         unsigned NumElements) const {
  // Check if we've already instantiated a vector of this type.
  SierraTypeMap::const_iterator i =
      SierraKey2Type.find(SierraKey(T.getTypePtr(), NumElements));
  ...
  if (const RecordType *RT = dyn_cast<RecordType>(T)) {
    ...
    for (RecordDecl::field_iterator i = OldRD->field_begin(),
                                    e = OldRD->field_end();
         i != e; ++i) {
      QualType NewT = getSierraVectorType(i->getType(), NumElements);
      FieldDecl *NewFD = FieldDecl::Create(
          *this, NewRD, i->getLocStart(), i->getLocation(), i->getIdentifier(),
          NewT, i->getTypeSourceInfo(), i->getBitWidth(), i->isMutable(),
          i->getInClassInitStyle());
    ...
    New =
        new (*this, TypeAlignment) SierraVectorType(T, NumElements, Canonical);
...
QualType
ASTContext::getDependentSizedSierraVectorType(QualType vecType, Expr *SizeExpr,
  ...
  if (Canon) {
    ...
    New = new (*this, TypeAlignment) DependentSizedSierraVectorType(
    ...
    if (CanonVecTy == vecType) {
      ...
      QualType Canon = getDependentSizedSierraVectorType(CanonVecTy, SizeExpr,
                                                         SourceLocation());
      New = new (*this, TypeAlignment) DependentSizedSierraVectorType(
          *this, vecType, Canon, SizeExpr, AttrLoc);
    }
  }
